...
public class BatchConfig {
	...
	public FlatFileItemReader<Invoice> reader() {
		...
		reader.setResource(new ClassPathResource("invoice.csv"));
	...
	@Bean
	public Step stepA() {
		return sbf.get("stepA").<Invoice, Invoice>chunk(2).reader(reader()).processor(processor()).writer(writer())
				.build();
	}
	...
	@Bean
	public Job jobA() {
		return jbf.get("jobA").incrementer(new RunIdIncrementer()).listener(listener()).start(stepA())
				// .next(stepB())
				// .next(stepC())
				.build();
	}
